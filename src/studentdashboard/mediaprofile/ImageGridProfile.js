...
export default function ImageGridProfile({ media = [], post, chats, setEditContent, setSelectedPost, selectedPost,
...
const shareToChat = async (chatId) => {
  ...
  return (
    <>
      <div className="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-4 mb-4 w-full">
        {media.slice(0, 4).map((img, i) => (
          <div className="relative">
          <img
            key={img.id}
            src={img.url}
            className="h-52 w-full  flex-1 rounded cursor-pointer"
            onClick={() => {
              setIndex(i);
              setOpen(true);
            }}
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              setOpenOptionId(prev => (prev === img.id ? null : img.id));
            }}
