...
export function CommentReportModal({ comment, onClose }) {
 ...
 const submitReport = async () => {
  ...
  const reportedUserId = comment?.user?.id;
  const targetId = comment?.id;

  if (!reportedUserId || !targetId) {
  ...
  setLoading(true);
  try {
    const res = await api.post("/api/comment/report", {
      comment_id: targetId,          // ✅ real comment/reply id
      reported_user_id: reportedUserId, // ✅ real user id
      reason,
      details,
    });
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white sm:w-96 w-72 rounded shadow p-4">
        ...
        <div className="flex justify-end text-sm hover:text-gray-800 gap-2 mt-4">
         <button onClick={onClose} className="text-white px-4 rounded py-2 bg-gray-800  ">
            Cancel
          </button>
          <button
            disabled={!reason}
