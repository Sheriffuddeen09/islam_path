...
useEffect(() => {
  ...
  fetchVideos();
   ...
   const currentPost = reels[currentIndex];
    ...
    onSwiping: (eventData) => {
        // Only allow vertical swiping
        if (
            Math.abs(eventData.deltaX) >
            Math.abs(eventData.deltaY)
        ) {
            return;
        }
    },
...
const onPlay = async () => {
  ...
  await api.post(`/api/post/${currentPost.id}/view`);
};
...
const handleShare = async (platform) => {
  ...
  await api.post(`/api/post/${currentPost.id}/share`);
};


const shareToChat = async (chatId) => {
  await api.currentPost(`/api/chats/${chatId}/messages`, {
    type: "link",
    body: shareUrl,
  });
    ...
    <div>
      <div className="lg:w-[400px] w-full border-l z-50">
        <div className="flex p-4 items-start justify-between">
              ...
              <PostOptions post={currentPost} chats={chats}/>
