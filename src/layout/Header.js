...
import { Book, BookOpen, BookTemplate, BookTemplateIcon, CreativeCommons, Download, EggFried, Globe, Home, LayoutDashboard, LibraryIcon, MagnetIcon, MessageCircle, MessageCircleIcon, PlayIcon, PlaySquare, Search, User, User2, User2Icon, Video, Videotape } from "lucide-react";
...
console.log('currentUser', user)
    ...
    const check = (
      ...
      {isLoggedin && user ? (
        ...
        <button onClick={() => setDashboardToggle(!dashboardToggle)} className='bg-black flex mx-auto z-50 hover:bg-gray-900 text-center rounded-full absolute top-6 right-0'>
    ...
    const checkMobile = (
      ...
      {isLoggedin && user ? (
        <div className='flex flex-row bg-gray-900 h-32 justify-between items-center p-3 rounded-lg'>
          ...
          <button onClick={() => setDashboardToggle(!dashboardToggle)} className='bg-black z-50 hover:bg-gray-900 text-center rounded-full'>
    ...
    return (
      <>
        <header className="z-50 bg-white  fixed w-full z-10 border-b- shadow px-3 sm:py-4 py-0.5 sm:mb-16 mb- ">
          ...
          <nav className='flex flex-row justify-between items-center lg:mx-7'>
            ...
            <div className=''> 
              <div className='sm:gap-8 gap-3 font-bold inline-flex '> 
                ...
                <Link to={'/'} className={`${homepage === '/' ? 'text-blue-600 hover:text-b-500' : 'text-gray-600 hover:text-gray-800'} sm:text-[13px] text-[11px]  rounded lg:p-2 px-1 py-2 
                ...
                {/* Friend */}
                <Link to={'/friend'} className={`${homepage === '/friend' ? 'text-blue-600 hover:text-b-500' : 'text-gray-600 hover:text-gray-800'} sm:text-[13px] text-[11px]  rounded lg:p-2 px-1 py-2 
                  transition-all duration-500 ease-in-out cursor-pointer about flex-col flex items-center gap-1`}> 
                ...
                {/* Get Mentor */}
                <Link to={'/get-mentor'} className={`${homepage === '/get-mentor' ? 'text-blue-600 hover:text-b-500' : 'text-gray-600 hover:text-gray-800'} sm:text-[13px] text-[11px]  rounded lg:p-2 px-1 py-2 
                  transition-all duration-500 ease-in-out cursor-pointer about flex-col flex items-center gap-1`}> 
                ...
                {/* Video */}
               <Link to={'/post/video'} className={`${homepage === '/post/video' ? 'text-blue-600 hover:text-b-500' : 'text-gray-600 hover:text-gray-800'} sm:text-[13px] text-[11px]  rounded lg:p-2 px-1 py-2 
                  transition-all duration-500 ease-in-out cursor-pointer about flex-col flex items-center gap-1`}> 
                ...
                {/* Quran */}
                <Link to={'/online-sale'} className={`${homepage === '/online-sale' ? 'text-blue-600 hover:text-b-500' : 'text-gray-600 hover:text-gray-800'} sm:text-[13px] text-[11px]  rounded lg:p-2 px-1 py-2 
