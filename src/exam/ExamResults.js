...
export default function ExamResults() {
  ...
  const handleDeletePop = () =>{
    ...
    const deleteResult = async (id) => {
      setDeleteLoading(id);
      try {
        await api.delete(`/api/exam-results/${id}`);
    ...
    link.href = url;
    link.setAttribute("download", "exam-result.pdf");
  ...
  const content = (
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-white w-full mx-auto text-center text-sm sm:text-[15px] p-2 font-semibold text-black sm:w-96 rounded-lg p-6 relative">
        ...
        <span className="text-center my-10">Are you sure you want to delete this exam, student won't be having access to the exam After Delete?</span>
  ...
  return (
      ...
      <div className="max-h-[60vh] overflow-y-auto space-y-4">
              ...
              {/* 🔽 SHOW CORRECT ANSWER ONLY ON WRONG SELECTION */}
              {answer &&
                answer.selected_answer !== q.correct_answer && (
                  <p className="ml-4 mt-2 text-sm text-green-600">
