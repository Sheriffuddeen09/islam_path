...
const CommentReportUser = () => {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const { data } = await api.get(`/comment/report/${commentId}`);
        setReport(data);
      } catch (error) {
        console.error(error);
      } finally {
  ...
  return (
    <div className="container mx-auto max-w-2xl mt-6 p-4 bg-white rounded shadow">
      <h2 className="text-2xl font-bold mb-4 text-gray-800">Comment/Reply Report</h2>

      <div className="flex items-center mb-4">
        <FaCommentDots className="w-8 h-8 text-purple-500 mr-2" />
        <div>
