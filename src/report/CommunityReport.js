...
export default function CommunityReport() {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const res = await api.get("/api/community/report");
        setReports(res.data || []);
      } catch (err) {
        ...
        <table className="min-w-full border-collapse">
          <thead className="bg-gray-100 text-gray-700">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-sm font-semibold">Community Channel Id</th>
          ...
          <tbody>
            {reports.map((r) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    navigate(`/community/report/${r.id}`)
                  }
                  className="
                    bg-blue-600
                    hover:bg-blue-700
      ...
      <div className="grid grid-cols-1 gap-4 md:hidden">
        {reports.map((r) => (
          ...
          >
            ...
            <p className="text-sm text-gray-700">
              <strong>Community Channel Id:</strong> {r.chat_id || "N/A"}
            </p>
