...
export default function CommunitySearch({
  ...
  const filteredCommunities = useMemo(() => {
    ...
    return members.filter((community) => {
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          ...
          <div className="bg-white w-full max-w-md h-[500px] rounded-xl shadow-lg flex flex-col">
            ...
            <div className="p-4 border-b flex justify-between items-center">
              <h2 className="font-semibold text-lg">Search Communities</h2>
              <button onClick={() => setShowCommunitySearchModal(false)}>
                ✕
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {filteredCommunities.length === 0 ? (
                <p className="text-center text-gray-700 p-5">
                  No members found
                </p>
              ) : (
                filteredCommunities.map((member) => (
                  <div
                    key={member.id}
