...
export function ForwardModalLargeScreen({
  ...
  // Reel states
  const [showReelReview, setShowReelReview] = useState(false);
  const [reelLoading, setReelLoading] = useState(false);
  const [activeMediaId, setActiveMediaId] = useState(null);
  const [reelDescriptions, setReelDescriptions] = useState({});
  ...
  */
  ...
  const reelMediaMessages = useMemo(() => {
  ...
  */
  ...
  const getMessageMediaUrl = (message) => {
    ...
    /*
     * Some APIs return file directly.
     */
    if (message.file) {
      return getUrl({
        file: message.file,
    ...
    /*
     * Some APIs return files array.
     */
    if (
      Array.isArray(message.files) &&
      message.files.length > 0
    ) {
      return getUrl(message.files[0]);
    }
  ...
  */
  ...
  const handleOpenReelReview = () => {
    ...
    /*
     * Prepare initial descriptions.
     *
     * If the message already has text,
     * use it as the starting description.
     */
    const initialDescriptions = {};

    mediaMessages.forEach((message) => {
      initialDescriptions[message.id] =
        message?.message || "";
    });

    setReelDescriptions(initialDescriptions);
  ...
  */
  ...
  const handleAddToReel = async () => {
    ...
    try {
       ...
       *
       * {
       *   15: "description for image 15",
       *   18: "description for video 18"
       * }
       */
      const mediaDescriptions = {};

      reelMediaMessages.forEach((message) => {
        mediaDescriptions[message.id] =
          reelDescriptions[message.id] || "";
      });
      ...
      /*
       * Give the new Reel to the parent immediately.
       */
      if (onReelAdded) {
        onReelAdded(newReel);
      }
  ...
  */
  ...
  const renderPreview = (msg) => {
            ...
            .map((file, i) => {
              ...
              return (
                ...
                >
                  ...
                  {file.type === "audio" && (
                    <div className="text-sm whitespace-nowrap font-bold text-gray-700 p-2">
                      🎵 {name}
                    </div>
                  )}
    ...
    /*
     * Fallback for a message that has file directly.
     */
    if (
      msg.file &&
      (msg.type === "image" ||
      ...
      return (
        <div className="w-16 h-16 rounded-lg bg-black flex items-center justify-center text-white">
          <Play className="w-6 h-6 fill-white" />
        </div>
      );
    }
  ...
  */
  ...
  const activeMedia = reelMediaMessages.find(
    (message) =>
      Number(message.id) ===
  ...
  */
  ...
  const activeDescription =
    activeMedia
      ? reelDescriptions[activeMedia.id] || ""
      : "";
    ...
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[999] bg-black/40">
        <div className="w-[95%] max-w-md bg-white p-4 rounded-2xl shadow-2xl">
          ...
          {/* ========================================================= */}
          {/* NORMAL FORWARD BUTTON      handleSendMeeting                               */}
          {/* ========================================================= */}
